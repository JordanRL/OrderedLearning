...
from analysis_tools.utils import flatten_grads, flatten_params
...
class HessianHook(InterventionHook):
    ...
    measurement. Uses ModelDataContext for checkpoint save/restore.
    """
    ...
    def intervene(self, run_ctx, model_ctx) -> dict[str, float]:
        ...
        metrics = self._compute_metrics(
            hv, baseline_grads, g_A_flat, g_A_norm, eta, run_ctx, model_ctx,
        )
    ...
    def _compute_metrics(
        ...
        if (
            ...
            and run_ctx.step is not None
            and self.step_schedule is not None
            and self.step_schedule.is_active(run_ctx.step - 1)
        ):
