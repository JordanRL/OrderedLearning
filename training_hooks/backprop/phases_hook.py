...
from framework.capabilities import HookNeeds, HookRequirements, ModelCapability, GradientAvailability
...
class PhasesHook(TrainingHook):
    ...
    requires = HookRequirements(
        model_capabilities=ModelCapability.PARAMETERS | ModelCapability.EMBEDDINGS,
    ...
    def compute(self, ctx, **state) -> dict[str, float]:
        ...
        grad_norm = flatten_grads(gradient_state.accumulated_grads).norm().item()
        ...
        acc = eval_metrics.metrics.get('validation_accuracy') if eval_metrics else None
        if acc is None:
            acc = 0
        if acc < 5:
