...
from framework.capabilities import HookNeeds, HookRequirements, TrainingParadigm, ModelCapability, GradientAvailability
...
class AdamDynamicsHook(InterventionHook):
    ...
    BackpropInterventionContext. Does not modify any training state.
    ...
    def intervene(self, ctx, model_ctx, **state) -> dict[str, float]:
        ...
        current_grads = self._get_current_gradient(state, model_ctx)
        ...
        # === Tier 3: Probe-dependent ===
        target_grad_flat = self._get_target_grad(state)
    ...
    def _get_current_gradient(self, state, model_ctx):
        ...
        if gradient_state is not None and gradient_state.accumulated_grads is not None:
            return gradient_state.accumulated_grads
    ...
    def _get_target_grad(self, state):
        """Get flattened target/probe gradient if available."""
        gradient_state = state.get('gradient_state')
        if gradient_state is None or gradient_state.target_grad is None:
            return None
        return flatten_grads(gradient_state.target_grad)
