...
class TrainingDiagnosticsHook(TrainingHook):
    ...
    def _accumulate(self, ctx, **state) -> dict[str, float]:
        ...
        loss = eval_metrics.metrics.get('loss') if eval_metrics else None
        if loss is not None:
        ...
        weight_norm = sum(
            p.data.norm().square() for p in model.parameters()
        ).sqrt().item()
        ...
        if grad_sq_terms:
            ...
            lr = model_state.lr if model_state.lr is not None else 0.0
