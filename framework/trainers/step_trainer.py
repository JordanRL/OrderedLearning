"""Step-based trainer. Scheduler steps every training step."""
...
class StepTrainer(Trainer):
    """Step-based training. Scheduler steps every training step.
            ...
            for step in range(s.start_counter, s.total + 1):
                ...
                if hook_manager and hook_manager.has_hooks_at(HookPoint.POST_STEP):
                    ...
                    hook_manager.fire(
                        ...
                        eval_metrics=EvalMetrics(metrics={'loss': float(result.loss)} if result.loss is not None else {}),
                ...
                # Post-step (e.g., phase transition)
                post_info = c.strategy.post_step(step, result)
                if post_info:
