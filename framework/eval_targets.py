...
def evaluate_all_targets(model, tokenizer, prepared_targets: list[dict]) -> dict[str, dict]:
    ...
    Returns:
        Dict mapping label -> {seq_prob, avg_target_prob, first_token_prob,
                                gen_text, loss, top_token_info}
    """
    results = {}
...
def build_sink_metrics(all_results: dict[str, dict], extra: dict | None = None) -> dict:
    ...
    for label, m in all_results.items():
        metrics[f'target/{label}/avg_target_prob'] = m['avg_target_prob']
        metrics[f'target/{label}/first_token_prob'] = m['first_token_prob']
        metrics[f'target/{label}/seq_prob'] = m['seq_prob']
        metrics[f'target/{label}/loss'] = m['loss']
        metrics[f'target/{label}/gen_text'] = m['gen_text']

    if len(all_results) == 1:
        # Backwards compat: flat keys for single target
        m = next(iter(all_results.values()))
        metrics['target_prob'] = m['avg_target_prob']
        metrics['first_token_prob'] = m['first_token_prob']
        metrics['seq_prob'] = m['seq_prob']
        metrics['loss'] = m['loss']
        metrics['gen_text'] = m['gen_text']
    else:
        # Multi-target: include primary (first) target as flat key for dashboards
        primary = next(iter(all_results.values()))
        metrics['primary_target_prob'] = primary['avg_target_prob']
