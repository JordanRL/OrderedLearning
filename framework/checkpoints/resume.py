...
def load_checkpoint(path, components, runner):
    """Load a training checkpoint and restore all state.

    Args:
        path: Path to the .pt checkpoint file.
        components: TrainingComponents to restore state into.
        runner: ExperimentRunner for loading experiment-specific state.
    """
    import torch
    checkpoint = torch.load(path, map_location='cpu', weights_only=False)

    components.load_state_dict(checkpoint['components'])
